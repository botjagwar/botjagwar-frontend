...
class LanguageColumnView(TableColumnView):
    server = None
    protocol = 'http'
    cache = {}
    def render(self):
        try:
            language_code = self.data[self.object_attribute_name]
            if language_code not in self.cache:
                url = f'{self.protocol}://{self.server}/language/{language_code}'
                response = requests.get(url)
                language_name = response.json()['english_name']
                self.cache[language_code] = language_name \
                    if language_name is not None \
                    else f'Unknown ({language_code})'
